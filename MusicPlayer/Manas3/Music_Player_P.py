...
# Function to play music based on song file name
def play_clicked_music(song_file_name):
    global current_music_index
    for index, file_path in enumerate(music_files):
        if os.path.basename(file_path) == song_file_name:
            current_music_index = index
            load_current_music()
            break

# Get the selected track (replace this with your code to get the selected track)
def get_selected_track():
    selected_track = "your_selected_track.mp3"  # Replace this with your code to get the selected track
    return selected_track

# Get the selected track
selected_track = get_selected_track()

# Play the selected track
play_clicked_music(selected_track)
...
while running:
    ...
    window.blit(shuffle_on_image if pygame.mixer.music.get_busy() else shuffle_off_image, shuffle_pos)
